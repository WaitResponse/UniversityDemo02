...
public class FruitDAOImpl extends BaseDAO<Fruit> implements FruitDAO {
	...
	public boolean addFruit(Fruit fruit) {
		return super.executeUpdate("insert into t_fruit values(null , ? , ? , ? , ? )" , fruit.getName() , fruit.getPrice() , fruit.getCount() , fruit.getRemark() ) ;
	}

	public boolean updateFruit(Fruit fruit) {
		return super.executeUpdate("update t_fruit set name = ? , price = ? , count = ? , remark = ? where id = ? " , fruit.getName() , fruit.getPrice() , fruit.getCount() , fruit.getRemark() , fruit.getId()) ;
	}
	...
	public boolean isFruitNameExists(String name) {
		String sql = "select count(*) from t_fruit where name = ? " ;
		return ((Integer)super.complexQuery(parser, sql, name)>0);
	}
}
