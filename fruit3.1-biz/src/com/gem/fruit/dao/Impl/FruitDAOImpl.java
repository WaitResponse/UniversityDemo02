...
public class FruitDAOImpl extends BaseDAO<Fruit> implements FruitDAO {
	...
	private class FruitParser implements IParser<Fruit>{
		...
		public Fruit loadRS(ResultSet rs) {//返回一个fruit 对象
			try {
				if(rs.next()){
					int id = rs.getInt(1);
					String fname = rs.getString(2);
	...
	public boolean updateFruit(Fruit fruit) {
		return super.executeUpdate("update t_fruit set name = ? , price = ? , count = ? , remark = ? where id = ? " , fruit.getFname() , fruit.getPrice() , fruit.getCount() , fruit.getRemark() , fruit.getId()) ;
	}
	...
	public int getFruitsCount() {//刚进来调用这个方法
